...
class Evaluator():
    ...
    def __init__(self, dataset, prompt_id, out_dir, dev_x ,devv_x, test_x ,testt_x , dev_y, test_y, dev_y_org, test_y_org):
        self.dataset = dataset
        self.prompt_id = prompt_id
        self.out_dir = out_dir
        self.dev_x, self.devv_x , self.test_x , self.testt_x = dev_x,  devv_x  , test_x , testt_x
    ...
    def evaluate(self, model, epoch, print_info=False):
        self.dev_loss, self.dev_metric = model.evaluate([self.dev_x , self.devv_x] , self.dev_y , batch_size=self.batch_size, verbose=0)
        
        self.test_loss, self.test_metric = model.evaluate([self.test_x,self.testt_x] ,  self.test_y, batch_size=self.batch_size, verbose=0)

        self.dev_pred = model.predict([self.dev_x , self.devv_x], batch_size=self.batch_size).squeeze()
        self.test_pred = model.predict([self.test_x ,self.testt_x ], batch_size=self.batch_size).squeeze()
    ...
    def print_info(self):
        ...
        print('[TEST]  QWK:  %.3f, LWK: %.3f, PRS: %.3f, SPR: %.3f, Tau: %.3f (Best @ %i: {{%.3f}}, %.3f, %.3f, %.3f, %.3f)' % (
    ...
    def print_final_info(self):
        ...
        print('  [DEV]  QWK: %.3f, LWK: %.3f, PRS: %.3f, SPR: %.3f, Tau: %.3f' % (self.best_dev[0], self.best_dev[1], self.best_dev[2], self.best_dev[3], self.best_dev[4]))
        print('  [TEST] QWK: %.3f, LWK: %.3f, PRS: %.3f, SPR: %.3f, Tau: %.3f' % (self.best_test[0], self.best_test[1], self.best_test[2], self.best_test[3], self.best_test[4]))